...
const Similar = ({ data }) => {
    ...
    return (
        <div className='text-start px-8 my-5'>
            <h1 className='text-2xl mb-5 md:mb-8'>Similar Movies</h1>
            <Carousel responsive={responsive}>
                {data?.length > 0 && data?.map((t) => {
                    return (
                        <div key={t.id} className='py-4 cursor-pointer mx-2' onClick={()=>navigate(`/movie/${t.id}`)}>
                            <img src={url.poster + t.poster_path} alt="" className='h-[240px] md:h-auto rounded-[15px]' />
